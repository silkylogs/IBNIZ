...
void clipboard_init() {
  if (!clipbrd.cbatom) {
    ...
    clipbrd.cbatom =
        XInternAtom(clipbrd.swi.info.x11.display, "IBNIZ_CB", False);
...
char *getcbtext(XSelectionEvent *e) {
  ...
  xdata = NULL;
  XGetWindowProperty(e->display, e->requestor, clipbrd.cbatom, 0, 0, False,
                     AnyPropertyType, &type, &format, &nitems, &remaining,
                     &xdata);
  ...
  xdata = NULL;
  XGetWindowProperty(e->display, e->requestor, clipbrd.cbatom, 0,
                     (remaining + 3) / 4, False, AnyPropertyType, &type,
                     &format, &nitems, &remaining, &xdata);
  ...
  if (lgt)
    XStoreBytes(clipbrd.swi.info.x11.display, xdata, lgt);
  ...
  if (e->syswm.msg->event.xevent.type == SelectionRequest) {
    ...
    if (req->target == XA_STRING) {
      XChangeProperty(clipbrd.swi.info.x11.display, req->requestor,
                      req->property, XA_STRING, 8, PropModeReplace, clipboard,
                      strlen(clipboard));
