...
void gen_nativeinit() {
  printf("#include <SDL/SDL.h>\n"
         "#include <math.h>\n"
         "\n"
         "#define ROR(a,s)   "
         "((((uint32_t)(a))>>(s))|(((uint32_t)(a))<<(32-(s))))\n"
         "#define IBNIZ_ROR(a,b) ROR((a),(((b)>>16)&31))\n"
         "#define IBNIZ_MUL(a,b) "
         "((((int64_t)((int32_t)(a)))*((int32_t)(b)))>>16)\n"
         "#define IBNIZ_DIV(a,b) "
         "((b)==0?0:((((int64_t)((int32_t)(a)))<<16)/((int32_t)b)))\n"
         ...
         "#define IBNIZ_ROR(a,b) ROR((a),(((b)>>16)&31))\n"
         "#define IBNIZ_SHL(a,b) "
         "(((b)&(32<<16))?((a)>>(((~(b))>>16)&31)):((a)<<((((b))>>16)&31)))\n"
...
void gen_mov_reg_reg_reg(int t, int s) { printf("%c=%c;\n", t + 'A', s + 'A'); }

void gen_mov_reg_imm(int t, int32_t i) { printf("%c=0x%X;\n", t + 'A', i); }
...
void gen_neg_reg_reg(int t, int s) { printf("%c=~%c\n", t + 'A', s + 'A'); }
...
void gen_push_reg(int s) { printf("stack[(++sp)&0x1FFFF]=%c;\n", s + 'A'); }

void gen_push_imm(int32_t i) { printf("stack[(++sp)&0x1FFFF]=0x%X;\n", i); }

void gen_pop_reg(int t) { printf("%c=stack[sp--];sp&=0x1FFFF;\n", t + 'A'); }

void gen_pop_noreg() { printf("sp=(sp-1)&0x1FFFF;\n"); }

void gen_dup_reg(int t) { printf("%c=stack[sp];\n", t + 'A'); }
...
void gen_rpop_noreg() { printf("rsp=(rsp-1)&0x7FFF;\n"); }
...
void gen_bne_reg_lab(int s, int l) { printf("if(%c) goto l%d;\n", s + 'A', l); }
...
void gen_label(int l) { printf("l%d:\n", l); }

void gen_jmp_lab(int l) { printf("goto l%d;\n", l); }

void gen_jmp_rstack() { printf("goto (main+rstack[rsp]);\n"); }
