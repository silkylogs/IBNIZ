    ...
    /* fill last 2 words to ease fetch */
    {
      ...
      if (!pad)
        ...
        vm.parsed_data[(vm.datalgt >> 5) + 1] =
            (vm.parsed_data[0] << (32 - pad)) | (vm.parsed_data[1] >> pad);
      }
    }
  }
...
void freereg(int reg) { gen.usedregs &= ~(1 << reg); }
...
flushstack() { gen_flushpartialstack(gen.gsp + 1); }
...
gen_loadimm(int val) { growstack(GSV_ABS, val); }
...
/*
// we must harmonize post-if and post-else gs structure (and post-a and post-b
as well)
// cond IF  a ELSE b ENDIF c
//        0  1    0 1     1
// cond IF a ENDIF
//        0 0     0
// trivial: just flushstack() at these points (before if-jump, before else-jump,
before endif-label)
*/
