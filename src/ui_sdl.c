...
void drawStatusPanel() {
  ...
  if (ui.runstat) {
    if (ui.mops > 0) {
      sprintf(buf, "%3.3f Mops%c", ui.mops, ui.benchmark_mode ? '!' : ' ');
      drawString(buf, 0, 30);
    }
    if (ui.fps > 0) {
  ...
  drawString(ui.audio_off ? "off" : (vm.audiomode ? "ster" : "mono"), 13, 31);
...
void getkeystates() {
  ...
  m = ((m & KMOD_CTRL) ? 64 : 0) | ((m & (KMOD_ALT | KMOD_META)) ? 32 : 0) |
      ((m & KMOD_SHIFT) ? 16 : 0) | (k[SDLK_UP] ? 8 : 0) |
      (k[SDLK_DOWN] ? 4 : 0) | (k[SDLK_LEFT] ? 2 : 0) | (k[SDLK_RIGHT] ? 1 : 0);
...
void ed_movecursor(char *target, char with_select) {
  if (!with_select)
    ...
    if (!ed.selectbase)
      ed.selectbase = ed.selectstart = ed.selectend = ed.cursor;
    if (target <= ed.selectbase)
      ed.selectstart = target;
    if (target >= ed.selectbase)
      ed.selectend = target;
  }
  ed.cursor = target;
...
void ed_up(char with_select) {
  ...
  if (x >= 32)
    ...
    if (p - pp - 1 < x)
      ...
      ed_movecursor(pp + x, with_select);
  } else
...
void ed_deleteselection() {
  ...
  if (ed.cursor > ed.selectend)
    ed.cursor -= gap;
  else if (ed.cursor > ed.selectstart)
    ed.cursor = ed.selectstart;
...
void interactivemode(char *codetoload) {
  ...
  ed_parallel.cursor = ed_parallel.selectstart = ed_parallel.selectend =
  ...
  for (;;) {
    ...
    if (prevtimevalue != t || e.type != SDL_NOEVENT) {
      ...
      DEBUG(stderr, "t:%x audio:%x playback:%x video:%x\n", t,
            (vm.audiotime) + (((vm.mediacontext == 1) ? vm.sp : vm.cosp) >> 10),
    ...
    if (e.type == SDL_KEYDOWN) {
      ...
      if (ui.opt_dumpkeys) {
        ...
        printf("%d %d %d %d\n", now - last, sym, e.key.keysym.unicode, mod);
        last = now;
      }
